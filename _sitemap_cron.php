...
//////////////////////////////////// Options ////////////////////////////////////
$url = 'http://yours-todo.tk/'; //The Url of the site - the last '/' is needed
...
function addDebug($msg){
	$_debugF = fopen('_debugs.txt','a');
	fwrite($_debugF,date('d-m-Y h:i:s') .": $msg \n");
	fclose($_debugF);
}
// File finding function.
function getFiles($cd) {
	$links = array();
	//Add the slash only if we are in a valid folder
 
	$directory = ($cd) ? "$cd/" : '';
 
	foreach(glob($directory . $GLOBALS['file_mask']) as $link) {
	...
	//Get All folders.	
	//GLOB_ONLYDIR not avalilabe on windows.
...
';
  ...
  <priority>$priority</priority>
	...
	if(!$hndl = fopen($sitemap_file,'w')) {
		...
		addDebug("Can't open sitemap file - '$sitemap_file'.\nDumping result to screen...\n<br /><br /><br />\n\n\n");
		addDebug('<textarea rows="25" cols="70" style="width:100%">'.$xml_string.'</textarea>');
	} else {
		addDebug( '<p>Sitemap was written to <a href="' . $url.$sitemap_file .'">'. $url.$sitemap_file .'></a></p>');
	...
	addDebug("$total_file_count $average");
?>